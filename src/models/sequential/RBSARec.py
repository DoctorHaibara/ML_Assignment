...
class RBSARec(SequentialModel, RBSARecBase):
    # 定义读取器和运行器
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    def parse_model_args(parser):
        # 解析模型参数
        parser = RBSARecBase.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        # 前向传播
        out_dict = RBSARecBase.forward(self, feed_dict)
        return {'prediction': out_dict['prediction']}
    
class RBSARecImpression(ImpressionSeqModel, RBSARecBase):
    # 定义读取器和运行器
    ...
    def forward(self, feed_dict):
        # 前向传播
        return RBSARecBase.forward(self, feed_dict)